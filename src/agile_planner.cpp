...
Eigen::VectorXd AgilePlanner::generateIndividualThrust(Eigen::Vector3d& acceleration, Eigen::Vector3d& current_omega) {
  ...
  /* std::cout << total_thrust << std::endl; */
...
/* processImpulse() //{ */
void AgilePlanner::processAngularSpeedImpulse(int window) {
  std::vector<laser_msgs::msg::ReferenceState> aux = full_trajectory_path_;
  int                                          n   = aux.size();

  for (int i = 1; i < n - 1; ++i) {
    int start = std::max(0 + 1, i - window / 2);
    int end   = std::min(n - 1, i + window / 2 + 1);
...
void AgilePlanner::processThrustImpulse(int window) {
  std::vector<laser_msgs::msg::ReferenceState> aux = full_trajectory_path_;
  int                                          n   = aux.size();
...
/*   if (full_trajectory_path_.empty() || window <= 1) { */
/*     return; // Proteção contra trajetórias vazias ou janelas sem sentido */
/*   } */
...
/*     double count = end - start; // Usando double direto para garantir a divisão correta */

/*     // Variáveis acumuladoras locais */
/*     double sum_angular_velocity_x = 0.0; */
/*     double sum_angular_velocity_y = 0.0; */
/*     double sum_angular_velocity_z = 0.0; */

/*     // Descobre quantos motores o ponto atual 'i' exige */
/*     int num_motors = aux[i].individual_thrust.data.size(); */
/*     std::vector<double> sum_individual_thrust(num_motors, 0.0); */

/*     // Faz UMA ÚNICA varredura na janela somando todos os eixos e motores de uma vez */
/*     for (int k = start; k < end; ++k) { */
/*       sum_angular_velocity_x += aux[k].twist.angular.x; */
/*       sum_angular_velocity_y += aux[k].twist.angular.y; */
/*       sum_angular_velocity_z += aux[k].twist.angular.z; */

/*       for (int j = 0; j < num_motors; ++j) { */
/*         // Blindagem contra lixo de memória: só soma se o ponto 'k' realmente tiver esse motor */
/*         if (j < (int)aux[k].individual_thrust.data.size()) { */
/*           sum_individual_thrust[j] += aux[k].individual_thrust.data[j]; */
/*         } else { */
/*           // Failsafe: Se o vizinho estiver sem dado de thrust, repete o do ponto alvo 'i' */
/*           // Isso evita que a média "caia" artificialmente. */
/*           sum_individual_thrust[j] += aux[i].individual_thrust.data[j]; */
/*         } */
/*       } */
/*     } */

/*     // Aplica a média (Moving Average) */
/*     full_trajectory_path_[i].twist.angular.x = sum_angular_velocity_x / count; */
/*     full_trajectory_path_[i].twist.angular.y = sum_angular_velocity_y / count; */
/*     full_trajectory_path_[i].twist.angular.z = sum_angular_velocity_z / count; */

/*     for (int j = 0; j < num_motors; ++j) { */
/*       full_trajectory_path_[i].individual_thrust.data[j] = sum_individual_thrust[j] / count; */
/*     } */
/*   } */
/* } */
//}
...
bool AgilePlanner::generateTrajectory(nav_msgs::msg::Odometry start_waypoint, laser_msgs::msg::PoseWithHeading end_waypoint, float speed, bool use_speed) {
  ...
  for (auto i = 0; i < (int)trajectory_time_.size(); i++) {
    ...
    if (i > 0 && i < (int)trajectory_time_.size() - 1) {
      ...
      std::cout << (mass_ * 9.81) / 4 << std::endl;
      ref.individual_thrust.data = std::vector<double>(4, (mass_ * 9.81) / 4);
      ref.use_individual_thrust  = true;
    }

    full_trajectory_path_.push_back(ref);
  }
  processAngularSpeedImpulse(300);
  ...
  for (auto i = 0; i < (int)trajectory_time_.size(); i++) {
    if (i > 0 && i < (int)trajectory_time_.size() - 1) {
      ...
      current_omega << full_trajectory_path_[i].twist.angular.x, full_trajectory_path_[i].twist.angular.y, full_trajectory_path_[i].twist.angular.z;

      Eigen::VectorXd individual_thrust               = generateIndividualThrust(acceleration, current_omega);
      full_trajectory_path_[i].individual_thrust.data = std::vector<double>(individual_thrust.data(), individual_thrust.data() + individual_thrust.size());
    }
  }
  /* processThrustImpulse(200); */
...
bool AgilePlanner::generateTrajectory(nav_msgs::msg::Odometry start_waypoint, std::vector<laser_msgs::msg::PoseWithHeading> waypoints, float speed) {
  ...
  current_anchor_time_ = -1.0;
  /* start_sim_time_ = start_waypoint.header.stamp.sec + (start_waypoint.header.stamp.nanosec * 10e-9); */
  /* std::cout << start_sim_time_ << std::endl; */
  is_hover_ = false;
  full_trajectory_path_.clear();
  full_trajectory_path_.shrink_to_fit();
...
std::vector<laser_msgs::msg::ReferenceState> AgilePlanner::getTrajectory(int qty_points, double current_ros_time) {
  ...
  double elapsed_time = std::abs(current_ros_time - start_sim_time_);
  ...
  /* while (trajectory_time_.size() > 2 && trajectory_time_[1] < elapsed_time - 5.0) { */
...
/*     if (state_prev.individual_thrust.data.size() == state_next.individual_thrust.data.size()) { */
/*       for (size_t i = 0; i < state_prev.individual_thrust.data.size(); ++i) { */
/*         double interpolated_thrust = state_prev.individual_thrust.data[i] + alpha * (state_next.individual_thrust.data[i] -
 * state_prev.individual_thrust.data[i]); */
/*         interp_state.individual_thrust.data.push_back(interpolated_thrust); */
/*       } */
/*     } else { */
...
bool AgilePlanner::isHover() {
  return is_hover_;
  /* if (full_trajectory_path_.size() < 2) { */
  /*   return true; */
  /* } else { */
  /*   return false; */
  /* } */
